...
<body class="bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-200">
    ...
    <!-- Navigation -->
    <nav class="fixed top-0 left-0 right-0 z-50 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-900 transition-colors">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                <div class="flex items-center gap-3">
                    ...
                    <span class="font-bold text-slate-900 dark:text-white text-lg">Virtual Pet Care</span>
                </div>
                <div class="hidden md:flex items-center gap-8">
                    <a href="#features" class="text-sm text-slate-600 dark:text-slate-300 hover:text-primary-600 dark:hover:text-primary-400 transition-colors">Features</a>
                    <a href="#how-it-works" class="text-sm text-slate-600 dark:text-slate-300 hover:text-primary-600 dark:hover:text-primary-400 transition-colors">How it works</a>
                    <a href="#testimonials" class="text-sm text-slate-600 dark:text-slate-300 hover:text-primary-600 dark:hover:text-primary-400 transition-colors">Testimonials</a>
                </div>
                <div class="flex items-center gap-3">
                    {{-- Dark mode toggle --}}
    ...
    <section class="pt-32 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div class="text-center max-w-4xl mx-auto">
            <div class="inline-flex items-center gap-2 bg-primary-50 dark:bg-primary-950/40 text-primary-700 dark:text-primary-300 rounded-full px-4 py-1.5 text-sm font-medium mb-6">
                <span class="w-2 h-2 bg-primary-600 dark:bg-primary-400 rounded-full animate-pulse"></span>
                Trusted by 10,000+ pet owners
            </div>
            <h1 class="text-5xl sm:text-6xl font-bold text-slate-900 dark:text-white leading-tight mb-6">
            ...
            <p class="text-sm text-slate-400 dark:text-slate-500 mt-4">Free forever for up to 3 pets</p>
        </div>
        ...
        <div class="mt-16 grid grid-cols-2 sm:grid-cols-4 gap-6 max-w-3xl mx-auto">
            @foreach([['10K+', 'Happy pet owners'], ['50K+', 'Reminders sent'], ['99%', 'Uptime'], ['4.9★', 'App rating']] as [$num, $label])
                <div class="text-center p-5 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-100 dark:border-slate-800/80">
                    <p class="text-2xl font-bold text-primary-600 dark:text-primary-400">{{ $num }}</p>
    ...
    <!-- Features -->
    <section id="features" class="py-20 bg-slate-50 dark:bg-slate-900/30 border-y border-slate-100 dark:border-slate-900">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                <h2 class="text-4xl font-bold text-slate-900 dark:text-white mb-4">Everything your pet needs</h2>
                <p class="text-xl text-slate-500 dark:text-slate-400 max-w-2xl mx-auto">A complete platform for managing your pet's health, care routines, and medical history.</p>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach([
                    ['🔔', 'Smart Reminders', 'Set up daily, weekly, or custom reminders for feeding, medication, grooming, and more. Never miss a care routine.', 'bg-primary-50 dark:bg-primary-950/50'],
                    ['🏥', 'Health Tracking', 'Log weight, medical records, vaccinations, and symptoms. Get a complete health timeline for each pet.', 'bg-teal-50 dark:bg-teal-950/50'],
                    ['📅', 'Appointment Calendar', 'Schedule and track vet appointments, grooming sessions, and vaccinations with a beautiful calendar view.', 'bg-purple-50 dark:bg-purple-950/50'],
                    ['💊', 'Medication Management', 'Track medications, dosages, and schedules. Get alerts when it\'s time for the next dose.', 'bg-orange-50 dark:bg-orange-950/50'],
                    ['📊', 'Analytics Dashboard', 'Visualize your pet\'s care consistency with charts and insights. See trends and patterns over time.', 'bg-blue-50 dark:bg-blue-950/50'],
                    ['🐾', 'Multi-Pet Support', 'Manage all your pets in one place. Each pet gets their own profile, health records, and reminders.', 'bg-pink-50 dark:bg-pink-950/50'],
                ] as [$icon, $title, $desc, $bg])
                    <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-100 dark:border-slate-800 hover:shadow-soft dark:hover:shadow-slate-950/40 transition-all duration-200 hover:-translate-y-0.5">
                        <div class="w-12 h-12 {{ $bg }} rounded-2xl flex items-center justify-center text-2xl mb-4 shadow-inner">{{ $icon }}</div>
    ...
    <section id="how-it-works" class="py-20">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                <h2 class="text-4xl font-bold text-slate-900 dark:text-white mb-4">Get started in minutes</h2>
                <p class="text-xl text-slate-500 dark:text-slate-400">Three simple steps to better pet care</p>
            </div>
                ...
                @foreach([
                    ...
                    <div class="text-center">
                        <div class="relative inline-flex">
                            ...
                            <div class="absolute -top-2 -right-2 w-6 h-6 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-full text-xs font-bold flex items-center justify-center">{{ $step }}</div>
                        </div>
                        <h3 class="text-xl font-semibold text-slate-900 dark:text-white mb-3">{{ $title }}</h3>
    ...
    <!-- Testimonials -->
    <section id="testimonials" class="py-20 bg-slate-50 dark:bg-slate-900/30 border-t border-slate-100 dark:border-slate-900">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                <h2 class="text-4xl font-bold text-slate-900 dark:text-white mb-4">Loved by pet owners</h2>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                @foreach([
                    ...
                    <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-100 dark:border-slate-800 shadow-sm">
                        <p class="text-slate-600 dark:text-slate-300 leading-relaxed mb-4">"{{ $quote }}"</p>
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-slate-400 dark:text-slate-500">{{ $role }}</p>
                            </div>
    ...
    <section class="py-20">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <div class="bg-gradient-to-r from-primary-600 to-secondary-500 rounded-3xl p-12 text-white shadow-xl shadow-primary-500/10">
                ...
                <a href="{{ route('register') }}" class="inline-flex items-center gap-2 bg-white text-primary-700 font-semibold px-8 py-4 rounded-2xl hover:bg-slate-100 transition-colors text-lg shadow-lg">
